...
namespace MemoTest
{
    internal class MemoMain
    {
        ...
        //수정할 글번호를 입력받아 수정할 글 1건을 얻어와서 화면에 표시하고
        // 비밀번호를 입력받아 memoList라는 List에 저장된 글 1건을 수정한다.
        private static void Update()
        {
            Console.Write("수정할 글 번호 >>");
            int idx = Convert.ToInt32(Console.ReadLine());
            if (memoList.Count() < idx)
            {
                Console.WriteLine(idx + "번 글이 존재하지 않습니다.");
            }
            else
            {
                MemoVo origin = memoList.SelectMemo(idx);
                Console.WriteLine("수정할 글 확인");
                Console.WriteLine(origin.ToString());

                //수정할 글의 비밀번호를 입력받는다.
                Console.Write("수정할 글 비밀번호 입력 >> ");
                string password = Console.ReadLine();
